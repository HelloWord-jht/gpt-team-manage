...
- 成员邮箱、上车日期、下车日期、付款状态管理
- 按月查询当月有效成员
- 搜索、地区筛选、状态筛选
- 按开通日期对应币种汇率换算人民币成本，并计算真实利润
- 月度结算快照，冻结当月账号、成员、收入、成本、利润和上下车记录
- 数据备份与恢复，支持手动创建备份并按备份恢复 `data` JSON
- 每天自动生成一份数据备份，自动检查上月结算快照
...
data/monthly-snapshots.json
data/backups/
```

其中 `renewal-actions.json` 保存续费工作台的当前周期处理记录。进入下一个续费周期后，车辆会自动重新出现在待处理列表中。

`monthly-snapshots.json` 保存月度结算快照。系统启动后会每天检查一次上个月是否已有快照；没有则自动生成，不会覆盖已有历史快照。

`data/backups/` 保存系统自动或手动创建的数据备份。备份只包含 `data` 目录顶层 JSON 文件，不包含 `.env`。
...
点击顶部的“账务中心”可以生成月度结算快照、查看历史快照、创建数据备份和恢复备份。

- “生成快照”会按当前选择月份重新生成该月快照。
- 自动快照只会补齐上个月缺失的记录，不会覆盖已有快照。
- “创建备份”会把当前 `data` 顶层 JSON 打包到 `data/backups/`。
- “恢复备份”会覆盖当前 `data` JSON，操作前建议先创建一份新备份。
